...
DELIMITER $$
CREATE PROCEDURE abastecer_producto(producto_id INT,cantidad INT)
BEGIN
	UPDATE Productos
    SET cantidad_actual = cantidad_actual + cantidad
    WHERE id = producto_id;
    
    SELECT CONTAC('Producto abastecido. Cantidad actual: ',cantidad_actual) AS Mensaje
    FROM Productos
    WHERE id = producto_id;
END $$
DELIMITER ;
...
DELIMITER $$
CREATE PROCEDURE cambiar_producto (producto_id INT,nuevo_nombre VARCHAR(50),nuevo_tipo ENUM('papeleria','supermercado','drogueria'),nuevo_precio_base DECIMAL(10,2))
BEGIN 
     ...
     IF NOT existe_nombre THEN 
            ...
            SELECT 'El prodcuto ha sido actualizado' AS mensaje;
...
BEGIN
    ...
    SELECT '
     ...
     SET opcion = 5; -- Cambiar este valor para probar las otras opciones
     AppMenuLoop: LOOP
     CASE opcion
		WHEN 1 THEN
